...
module adn_axi_fifo_tb;
  ...
  `AXI_RSP_T(axi, AXI_ID_WIDTH, AXI_DATA_WIDTH, AXI_USER_WIDTH)
  ...
  `GEN_DRIVE_RSP_TASK(b, axi_b_t)
  `GEN_DRIVE_RSP_TASK(r, axi_r_t)

  `GEN_CHECK_REQ_TASK(aw, axi_aw_t)
  `GEN_CHECK_REQ_TASK(w, axi_w_t)
  `GEN_CHECK_REQ_TASK(ar, axi_ar_t)
  `GEN_CHECK_RSP_TASK(b, axi_b_t)
  `GEN_CHECK_RSP_TASK(r, axi_r_t)
  ...
  task automatic tc_001_basic_aw();
    axi_aw_t item;
    mst_rsp_i.aw_ready = 1'b1;  // output side always ready, no backpressure
    ...
    if (aw_ref_q.size() == 0) note_case(1);
    else note_case(0);
  endtask
  ...
  task automatic tc_002_aw_fifo_full();
    ...
    mst_rsp_i.aw_ready <= 1'b0;

    for (push_count = 0; push_count < AW_FIFO_DEPTH; push_count++) begin
      item = make_aw_item(push_count);
      drive_aw(item);
      $display("T=%0t: pushed #%0d, ready=%b", $time, push_count + 1, slv_rsp_o.aw_ready);
    end
    @(posedge clk_i);
    ...
    end else note_case(0);

    mst_rsp_i.aw_ready <= 1'b1;
    ...
    if (aw_ref_q.size() != 0)
      $error(
          "[TC_002] flush incomplete - aw_ref_q still has %0d entries, expected 0", aw_ref_q.size()
      );
  endtask
  ...
  task automatic tc_003_w_fifo_full();
    ...
    for (push_count = 0; push_count < W_FIFO_DEPTH; push_count++) begin
      ...
      item.user = $bits(item.user)'(0);
      drive_w(item);
      $display("T=%0t: pushed #%0d, ready=%b", $time, push_count + 1, slv_rsp_o.w_ready);
    end
    @(posedge clk_i);
    if (slv_rsp_o.w_ready === 1'b0) note_case(1);
    else note_case(0);

    mst_rsp_i.w_ready <= 1'b1;
    ...
    if (w_ref_q.size() != 0)
      $error(
          "[TC_003] flush incomplete - w_ref_q still has %0d entries, expected 0", w_ref_q.size()
      );
  endtask
  ...
  task automatic tc_004_ar_fifo_full();
    ...
    for (push_count = 0; push_count < AR_FIFO_DEPTH; push_count++) begin
      ...
      item.user   = $bits(item.user)'(0);
      drive_ar(item);
      $display("T=%0t: pushed #%0d, ready=%b", $time, push_count + 1, slv_rsp_o.ar_ready);
    end
    @(posedge clk_i);
    if (slv_rsp_o.ar_ready === 1'b0) note_case(1);
    else note_case(0);

    mst_rsp_i.ar_ready <= 1'b1;
    ...
    if (ar_ref_q.size() != 0)
      $error(
          "[TC_004] flush incomplete - ar_ref_q still has %0d entries, expected 0", ar_ref_q.size()
      );
  endtask
  ...
  task automatic tc_005_r_fifo_full();
    ...
    @(posedge clk_i);
    if (mst_req_o.r_ready === 1'b0) note_case(1);
    else note_case(0);

    slv_req_i.r_ready <= 1'b1;
    repeat (R_FIFO_DEPTH + 2) @(posedge clk_i);

    if (slv_rsp_o.r_valid !== 1'b0)
      $error("[TC_005] flush incomplete - r_valid still high after expected flush time");
    if (r_ref_q.size() != 0)
      $error(
          "[TC_005] flush incomplete - r_ref_q still has %0d entries, expected 0", r_ref_q.size()
      );
  endtask
  ...
  task automatic tc_006_b_fifo_full();
    ...
    @(posedge clk_i);
    if (mst_req_o.b_ready === 1'b0) note_case(1);
    else note_case(0);

    slv_req_i.b_ready <= 1'b1;
    repeat (B_FIFO_DEPTH + 2) @(posedge clk_i);

    if (slv_rsp_o.b_valid !== 1'b0)
      $error("[TC_006] flush incomplete - b_valid still high after expected flush time");
    if (b_ref_q.size() != 0)
      $error(
          "[TC_006] flush incomplete - b_ref_q still has %0d entries, expected 0", b_ref_q.size()
      );
  endtask
  ...
  task automatic tc_008_aw_reset_midtransfer();
    ...
    if (mst_req_o.aw_valid === 1'b0 && slv_rsp_o.aw_ready === 1'b1) note_case(1);
    else note_case(0);
  ...
  task automatic tc_009_cross_channel_independence();
    ...
    end_time = $time;
    $display("T=%0t: [TC_009] W+AR completed in %0t while AW stalled", end_time,
             end_time - start_time);

    if (w_ref_q.size() == 0 && ar_ref_q.size() == 0) note_case(1);
    else note_case(0);
  ...
  task automatic tc_010_aw_randomized_backpressure();
    ...
    fork
      begin : rand_ready_gen
        forever begin
          @(posedge clk_i);
          mst_rsp_i.aw_ready <= $urandom_range(0, 1);
        end
      end
      begin
